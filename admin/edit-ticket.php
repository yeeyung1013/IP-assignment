...
if (isset($_POST['btnSubmit'])){
    ...
    if(count($finalErrorMessage) > 0){
        ...
        $updateCommand = "UPDATE ticket SET EventName='$eventName',TicketDate='$ticketDate',TicketTime='$ticketTime', TotalSeat='$totalSeat', goldPrice='$goldPrice', silverPrice='$silverPrice', bronzePrice='$bronzePrice' WHERE TicketID='$ticketId'";
        
        $result = mysqli_query($dbConnection, $updateCommand);
        
        echo "<div class='question'>";
        echo "Ticket has been update successfully. [<a href='ticket.php'>Ticket List</a>]";
        echo "</div>";
    }
}
...
<form method="POST" action="">
    <table>
      <div class = "inputBox">
        <h1>EVENT NAME</h1>
            <input name="eventName" id="eventNameBox" type="text" maxlength="40" placeholder="E.g. Valorant Event" required
                   value="<?php global $eventName; echo $eventName;?>"/>
            <i></i>
         </div>
        <div class = "box"> 
          <h1>TICKET DATE 📅</h1>
             <input type="date" name="ticketDate"  id="dateBox" required="true"
                    value="<?php global $ticketDate; echo $ticketDate;?>"/>
             <i></i>
        </div>
        <div class = "box">
        <h1>TICKET TIME ⌚</h1>
             <input type="time" name="ticketTime"  id="timeBox" required="true"
                    value="<?php global $ticketTime; echo $ticketTime;?>"/>
             <i></i>
        </div>
        <div class = "inputBox">
        <h1>TOTAL SEAT 💺</h1>
            <input name="totalSeat" id="seat" type="type" maxlength="11" required="true"
                   value="<?php global $totalSeat; echo $totalSeat;?>"/>
            <i></i>
        </div>
        <div class = "inputBox">
         <h1>GOLD TICKET PRICE (RM) 🎫</h1>
           <input type="type" name="goldPrice"  id="goldticket" required="true"
                  value="<?php global $goldPrice; echo $goldPrice;?>"/>
           <i></i>
        </div>
       <div class = "inputBox">
        <h1>SILVER TICKET PRICE (RM) 🎫</h1>
            <input type="type" name="silverPrice"  id="silverticket" required="true"
                   value="<?php global $silverPrice; echo $silverPrice;?>"/>
            <i></i>
        </div>
        <div class = "inputBox">
             <h1>BRONZE TICKET PRICE (RM) 🎫</h1>
            <input type="type" name="bronzePrice"  id="bronzeticket" required="true"
